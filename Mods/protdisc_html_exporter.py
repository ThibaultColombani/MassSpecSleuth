...
"""
protdisc_html_exporter.py - Orchestrator for the ProteomeDiscoverer HTML QC report.

Same tab architecture as fragpipe_html_exporter, sage_html_exporter, etc.
Tabs that require DIA-NN-specific data (fill_times, features, Precursor.Quantity)
are hidden automatically by their own is_visible() guards.

Tabs shown for a typical PD run:
  Files, Summary, Identifications, Chromatography
  Labeling (only when an amine-reactive tag is detected)
  Ion Sampling / Features / Plex Diagnostic — conditional on data availability
"""
...
def export_html_report(processor, output_path: str,
                       title: str = "ProteomeDiscoverer QC Report") -> None:
    """
    Build and write the ProteomeDiscoverer QC HTML report.

    Parameters
    ----------
    processor : ProtdiscProcessor | CombinedProcessor
