...
"""
diann_html_exporter.py - Orchestrator for the DIA-NN interactive HTML QC report.

Imports all tab modules, calls compute() on each, assembles the full HTML.
Each tab module is independent and can be added / removed / replaced without
touching this file.

Tab modules must export:
    TAB_ID    : str
    TAB_LABEL : str
    is_visible(processor) -> bool
    compute(processor)    -> dict
    html_content(data)    -> str   (inner HTML for the tab-content div)
    javascript()          -> str   (JS init function for this tab)
"""
...
def export_html_report(processor: DiannProcessor, output_path: str) -> None:
    """
    Build and write the DIA-NN QC HTML report.

    Parameters
    ----------
    processor : DiannProcessor
        Must already have .normalize() called.
    output_path : str
...
<title>MSS — DIA-NN QC Report</title>
...
document.addEventListener('DOMContentLoaded', () => {{
  buildFileList();
  // show first non-files tab's chart on load if summary is present
...
def _camel(tab_id: str) -> str:
    """Convert tab_id like 'ion_sampling' → 'IonSampling' for JS function names."""
