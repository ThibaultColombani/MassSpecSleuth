...
"""
fragpipe_html_exporter.py - Orchestrator for the FragPipe interactive HTML QC report.

Same tab architecture as sage_html_exporter, diann_html_exporter, etc.
Tabs requiring DIA-NN/Jmod-specific data (fill_times, features, plex_report,
Precursor.Quantity) are hidden automatically by their own is_visible() guards.

Tabs shown for a typical FragPipe (DDA) run:
  Files, Summary, Identifications, Chromatography
  Labeling (only when PSMtag data detected)
  Ion Sampling / Features / Plex Diagnostic — hidden (no DIA data; no Precursor.Quantity)
"""
...
def export_html_report(processor, output_path: str,
                       title: str = "FragPipe QC Report") -> None:
    """
    Build and write the FragPipe QC HTML report.

    Parameters
    ----------
    processor : FragpipeProcessor | CombinedProcessor
